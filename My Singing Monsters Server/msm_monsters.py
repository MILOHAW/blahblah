...
import time
from msm_box import (
    apply_awakened_box_state, apply_inactive_box_state, build_placeholder_box_monster,
    is_amber_no_vessel_island, is_box_monster_entity, requires_direct_placement,
    requires_direct_placement_on_purchase,
)

logger = logging.getLogger("msm.monsters")

# --- Time launcher feature: auto-level newly bought/placed monsters ---
# The launcher writes auto_monster_level.json next to the server modules:
#   {"enabled": true, "level": 12}
# When enabled, every freshly hatched/placed monster is created at that level
# (clamped to the monster's own max level) instead of the default level 1.
_AUTO_LEVEL_CFG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "auto_monster_level.json")

def _auto_monster_level():
    try:
        with open(_AUTO_LEVEL_CFG, encoding="utf-8") as _fh:
            _d = json.load(_fh)
        if _d.get("enabled"):
            return max(1, min(20, int(_d.get("level", 1))))
    except Exception:
        pass
    return None
...
from msm_playerdata import (
    SFSLong, action_result, add_actual_currencies, charge_entity_cost, create_player_properties,
...
def _add_collected_currency(player_object, collection_type, amount):
    ...
    if player_key == "coins":
        # NEXTSTARS capped this just under int32 for the mobile client; PC 5.5.0 carries
        # currency as LONG, so the low cap only served to clamp a maxed balance back down.
        updated = min(_COIN_CAP, updated)
...
def _find_egg(island, user_egg_id):
    normalized = _normalize_user_egg_id(user_egg_id)
    for egg in island.get("eggs") or []:
        if egg is None:
            continue
        egg_id = _normalize_user_egg_id(egg.get("user_egg_id"))
        if egg_id == normalized:
            return egg
    return None
...
def _find_direct_egg_monster(player_object, user_egg_id):
    normalized = _normalize_user_egg_id(user_egg_id)
    if normalized <= 0:
        return None
    direct_key = _DIRECT_PLACE_EGG_MAP.get(normalized)
    if direct_key is not None:
        return direct_key
    string_key = _DIRECT_PLACE_EGG_MAP.get(str(normalized))
    ...
    for island in player_object.get("islands") or []:
        ...
        for egg in island.get("eggs") or []:
            if egg is None:
                continue
            if _normalize_user_egg_id(egg.get("user_egg_id")) == normalized:
                value = egg.get("monster")
                if value not in (None, 0):
                    return value
    latest_egg_id = int(player_object.get("last_user_egg_id", 0) or 0)
    if normalized <= latest_egg_id:
        for island in player_object.get("islands") or []:
            if not isinstance(island, dict):
                continue
            direct_candidates = []
            for egg in island.get("eggs") or []:
                if egg is None:
                    continue
                structure = egg.get("structure")
                if structure in (0, None, "0"):
                    direct_candidates.append(egg)
            if len(direct_candidates) == 1:
                monster = direct_candidates[0].get("monster")
                if monster not in (None, 0):
                    return monster
    return None
_COIN_CAP = int(os.environ.get("COIN_CAP") or 9999999999)
_DIRECT_PLACE_EGG_MAP = {}


def _normalize_user_egg_id(value):
...
def _build_ms(monster_id):
    """Egg hatch duration in ms. INSTANT_HATCH=1 makes every egg hatch immediately, whether it
    came from the shop, breeding, or a box."""
    if os.environ.get("INSTANT_HATCH") == "1":
        return 0
...
def buy_egg(username, params):
    ...
    # Validate that the monster exists
    if definition is None:
        logger.warning("buy_egg: monster_id=%d has no definition, trying island-specific resolution", monster_id)
        resolved = resolve_monster_for_island(monster_id, island_type)
        if resolved == monster_id or resolved is None or resolved == 0:
            logger.error("buy_egg: could not resolve monster_id=%d to a valid monster on island_type=%d", monster_id, island_type)
            return {"success": False, "error": "invalid_monster_id"}, {}
        monster_id = resolved
        definition = get_monster_definition(monster_id)
    ...
    # Wublin Island (and friends) deliberately have NO nursery - that is the whole gimmick - so an
    # egg there has nowhere to go and the purchase used to fail with the client reporting an egg
    # already in the nursery. Fall back to direct placement only when the island truly has no
    # nursery. Islands that DO have one keep the normal flow, which is what real captures show
    # even for box monsters (gs_buy_egg 238: Wubbox Earth Island -> structure 2, a nursery).
    _island_has_nursery = _find_nursery(island, 0) is not None
    if (requires_direct_placement_on_purchase(get_monster_definition(monster_id), island_type,
                                              _island_has_nursery)
            or not _island_has_nursery):
        island_uid = island.get("user_island_id", 1000 + island_type)
        now = int(time.time() * 1000)
        build_ms = _build_ms(monster_id)
        ...
        _DIRECT_PLACE_EGG_MAP[next_egg_id] = monster_id
        user_egg = {
        ...
        # Direct-purchase eggs must remain discoverable from the island itself during the same
        # runtime session, even if the one-off map is lost or the client sends the ID as a string.
        eggs = island.setdefault("eggs", [])
        eggs.append(user_egg)
        logger.info("buy_egg direct placement: next_egg_id=%d, monster_id=%d, island_type=%d, eggs_count=%d", 
                    next_egg_id, monster_id, island_type, len(eggs))
        charge_entity_cost(player_object, get_monster_definition(monster_id))
        import msm_rewardtracks
        properties = msm_rewardtracks.properties_with_encore(player_object, 2, 2)
        save_player(username, root)
        logger.info("buy_egg: saved player with direct egg %d for monster %d", next_egg_id, monster_id)
        result = {
...
def _build_hatched_monster(monster_id, island, island_type, user_monster_id, pos_x, pos_y, flip, now, in_hotel=0):
    ...
    _lvl = _auto_monster_level()
    if _lvl is not None and isinstance(monster, dict) and "level" in monster:
        _maxlvl = get_max_monster_level(monster_id) or 20
        monster["level"] = min(_lvl, _maxlvl)
    return monster
def hatch_egg(username, params):
    pos_x = params.get("pos_x", 0)
    pos_y = params.get("pos_y", 0)
    flip = params.get("flip", 0)
    user_egg_id = _normalize_user_egg_id(params.get("user_egg_id", params.get("userEggId", 0)))
    logger.info("hatch_egg: user_egg_id=%d, pos=(%d,%d)", user_egg_id, pos_x, pos_y)
    root, player_object = load_player(username)
    island = find_island(player_object, get_active_island_id(player_object))
    if island is None or _find_egg(island, user_egg_id) is None:
        ...
        if by_egg is not None:
            logger.info("hatch_egg: found egg on different island")
            island = by_egg
    if island is None:
        direct_key = None
        if user_egg_id in _DIRECT_PLACE_EGG_MAP:
            direct_key = user_egg_id
        elif str(user_egg_id) in _DIRECT_PLACE_EGG_MAP:
            direct_key = str(user_egg_id)
        if direct_key is not None:
            logger.info("hatch_egg: found in direct map, key=%s", direct_key)
            island = find_island(player_object, get_active_island_id(player_object))
        else:
            logger.warning("hatch_egg: egg %d not found in islands or direct map, direct_map keys: %s", user_egg_id, list(_DIRECT_PLACE_EGG_MAP.keys()))
            return {"success": False, "user_egg_id": SFSLong(user_egg_id)}, {}
    eggs = island.setdefault("eggs", [])
    logger.info("hatch_egg: island has %d eggs", len(eggs))
    matched_egg = _find_egg(island, user_egg_id) or _find_fallback_egg(eggs)
    island_type = island_type_of(island) or 1
    nursery = None
    # A direct placement can be a runtime-only egg that was purchased without a nursery slot.
    # If the egg was already paid for during buy_egg, it is resolved from the runtime map here.
    direct_purchase = False
    if matched_egg is None:
        logger.info("hatch_egg: matched_egg is None, checking direct map for user_egg_id=%d", user_egg_id)
        direct_key = None
        if user_egg_id in _DIRECT_PLACE_EGG_MAP:
            direct_key = user_egg_id
        elif str(user_egg_id) in _DIRECT_PLACE_EGG_MAP:
            direct_key = str(user_egg_id)
        if direct_key is not None:
            monster_id = _DIRECT_PLACE_EGG_MAP.pop(direct_key, user_egg_id)
            logger.info("hatch_egg: resolved from direct map: monster_id=%d", monster_id)
        else:
            monster_id = _find_direct_egg_monster(player_object, user_egg_id)
            if monster_id is not None:
                direct_purchase = True
                logger.info("hatch_egg: found direct egg monster: monster_id=%d", monster_id)
            else:
                candidate_definition = get_monster_definition(user_egg_id)
                if user_egg_id > 0 and candidate_definition is not None:
                    monster_id = user_egg_id
                    direct_purchase = True
                    logger.info("hatch_egg: using user_egg_id as monster_id (direct purchase): %d", monster_id)
                else:
                    logger.error("hatch_egg: could not resolve monster for user_egg_id=%d, candidate_definition exists: %s", user_egg_id, candidate_definition is not None)
                    return {"success": False, "error": "monster_not_available", "user_egg_id": SFSLong(user_egg_id)}, {}
    else:
        monster_id = max(1, matched_egg.get("monster", 3) or 3)
        nursery_id = matched_egg.get("structure", 0)
        logger.info("hatch_egg: matched_egg found, monster_id=%d (from egg.monster), nursery_id=%d", monster_id, nursery_id)
    ...
    # Validate and resolve monster_id on the target island
    if get_monster_definition(monster_id) is None:
        logger.warning("hatch_egg: monster_id=%d has no definition, trying to resolve for island_type=%d", monster_id, island_type)
        resolved = resolve_monster_for_island(monster_id, island_type)
        if resolved is not None and resolved > 0 and get_monster_definition(resolved) is not None:
            logger.info("hatch_egg: resolved monster_id from %d to %d", monster_id, resolved)
            monster_id = resolved
        else:
            logger.error("hatch_egg: could not resolve monster_id=%d for island_type=%d", monster_id, island_type)
            return {"success": False, "error": "monster_not_available", "user_egg_id": SFSLong(user_egg_id)}, {}
    
    definition = get_monster_definition(monster_id)
    if definition is None:
        logger.error("hatch_egg: no definition found for monster_id=%d after validation", monster_id)
        result = {
    ...
    # Wublins/Wubboxes placed this way must arrive as the DORMANT stone version that has to be
    # filled before it wakes. _build_hatched_monster always makes an ordinary awake monster, and
    # this path never goes through build_placeholder_box_monster, so the box state is applied here.
    # AUTO_WAKE_BOXES=1 restores the old "already awake" behaviour.
    # AUTO_AWAKEN_BOX=1 automatically awakens box monsters after hatching without requiring user action.
    monster_update = None
    if is_box_monster_entity(definition):
        if os.environ.get("AUTO_WAKE_BOXES") == "1" or os.environ.get("AUTO_AWAKEN_BOX") == "1" or is_amber_no_vessel_island(island_type):
            apply_awakened_box_state(monster, definition, island_type)
            # If auto-awakening, send the awakened state update to the client
            if os.environ.get("AUTO_AWAKEN_BOX") == "1":
                monster_update = {
                    "user_monster_id": SFSLong(user_monster_id),
                    "awakened": True,
                    "inactive_box_monster": False,
                    "is_inactive_box_monster": False,
                    "box_monster": False,
                    "is_box_monster": False,
    ...
    if direct_purchase:
        charge_entity_cost(player_object, definition)
    import msm_rewardtracks
    properties = msm_rewardtracks.properties_with_encore(player_object, 2, 5)
    save_player(username, root)
    # Shape the reply like the real server's. Captured 2026-08-09 from a live Wublin placement
    # (game_capture/session_002.bin): directPlace is TRUE for a box monster even when the island
    # has a nursery, and the monster object carries five fields we never computed. The definitions
    # do not supply them, so these are structural defaults - collection_type in particular varies
    # per Wublin on the real server ('food' for Yuork, 'ethereal_currency' for Flick) and we have
    # no source for it, so it is a guess.
    _is_box = is_box_monster_entity(definition)
    response_monster = dict(monster)
    if _is_box:
        response_monster.setdefault("collection_type", "food")
        response_monster.setdefault("random_underling_collection_min", 0)
        response_monster.setdefault("underling_collection_happiness", 0)
        response_monster.setdefault("has_evolve_flexeggs", "[]")
        response_monster.setdefault("has_evolve_reqs", "[]")
    result = {
        "success": True, "create_in_storage": False,
        "directPlace": bool(nursery is None or _is_box),
        "user_egg_id": SFSLong(user_egg_id), "island": SFSLong(island_uid),
        "properties": properties,
        "monster": response_monster,
    }
    return result, monster_update
...
def send_to_magical_nexus(username, params):
    ...
    player_object["last_user_egg_id"] = next_egg_id
    now = int(time.time() * 1000)
    build_ms = _build_ms(monster_type)
    hatches_on = now if build_ms <= 0 else now + build_ms
