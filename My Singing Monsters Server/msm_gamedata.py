...
_STRUCTURE_DB_NAMES = [
    "db_structures", "db_structure", "db_structure_2", "db_structure_3", "db_structure_4",
...
def _normalize_monster_market_visibility(entry):
    if not isinstance(entry, dict):
        return entry
    name = str(entry.get("name") or "")
    class_name = str(entry.get("class") or "")
    is_rare_or_epic = "RARE" in class_name or "RARE" in name or "EPIC" in class_name or "EPIC" in name
    if is_rare_or_epic:
        entry["view_in_market"] = 1
    return entry
...
def _build_monster_defs():
    ...
    for data in _load_db_chunk_chain("db_monster"):
        for entry in data.get("monsters_data") or []:
            ...
            if mid is not None:
                defs[mid] = _normalize_monster_market_visibility(entry)
    return defs
...
def _island_theme_defs():
    """Returns all theme definitions forced to be active, available, and unlocked year-round."""
    global _island_theme_defs_cache
    if _island_theme_defs_cache is None:
        data = load_db_json("db_island_themes") or {}
        themes = data.get("island_theme_data") or []
        for theme in themes:
            theme["is_available"] = 1
            theme["available"] = 1
            theme["in_shop"] = 1
            theme["event_active"] = 1
            theme["cost_diamonds"] = 0
            theme["price"] = 0
            theme.pop("start_date", None)
            theme.pop("end_date", None)
            theme.pop("event_id", None)
        _island_theme_defs_cache = themes
...
def _breeding_boost_multiplier():
    """Forces breeding event boosts to be active year-round."""
    return 100
...
def get_active_timed_events():
    """Returns all timed events overridden to be active permanently."""
    data = load_db_json("gs_timed_events") or {}
    events = data.get("timed_event_list") or []
    now = int(time.time() * 1000)
    far_future = now + 10 * 365 * 24 * 3600 * 1000  # +10 years
    for event in events:
        event["start_date"] = now - 86400000
        event["end_date"] = far_future
        event["active"] = 1
