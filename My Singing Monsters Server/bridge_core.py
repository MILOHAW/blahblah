...
# Currency override intentionally disabled. Real player values must be respected.
def _apply_test_currency_overrides(player_object):
    return player_object

# Optional captured-player override. This must be explicitly enabled for local testing only.
def _inject_captured_player_data():
    if not hasattr(msm_playerdata, '_original_load_player'):
        msm_playerdata._original_load_player = msm_playerdata.load_player
    msm_playerdata._captured_player_data = None

    def _load_player_with_captures(username):
        if os.environ.get('ALLOW_CAPTURED_PLAYER_DATA', '0').lower() in {'1', 'true', 'yes', 'on'}:
            if msm_playerdata._captured_player_data:
                _player = json.loads(json.dumps(msm_playerdata._captured_player_data))
                logger.info('load_player(%s): using captured data without currency overrides', username)
                return _player
        return msm_playerdata._original_load_player(username)

    msm_playerdata.load_player = _load_player_with_captures
...
def locate_base():
    ...
    for _d in (
        Path(r'D:\Next-Private-Server-main\My Singing Monsters Server'),
        Path(r'E:\Next-Private-Server-main\My Singing Monsters Server'),
    ):
        if _d.exists() and ((_d / 'Config.json').exists() or (_d / 'config.json').exists()):
            return _d.resolve()
    return _a.resolve()
...
async def initiate_purchase(request: Request):
    _a = await params(request)
    logger.info('initiate_purchase params=%s', sorted(_a.keys()))
    return ok({'purchase_initiated': False, 'processed': False})
...
def _download_entries():
    ...
    for node in root.findall('.//Download'):
        ...
        try:
            ...
            logger.error('failed to compute checksum for %s: %s', file_name, e)
            continue
        entries.append({
            'localName': file_name,
...
# Load captured player data from Captures directory
def _locate_captures_dir():
    for _base in [Path(r'E:\Next-Private-Server-main'), Path(r'D:\Next-Private-Server-main')]:
        _cap_dir = _base / 'Captures' / '1' / 'msm_json'
        if _cap_dir.exists():
            return _cap_dir
    return None

def _load_captured_player_data():
    _cap_dir = _locate_captures_dir()
    if not _cap_dir:
        logger.warning('Captures directory not found, will fall back to Nextstars.json')
        return None
    _player_file = _cap_dir / '69_gs_player.json'
    if not _player_file.exists():
        logger.warning('Captured player file not found: %s', _player_file)
        return None
    try:
        _data = json.loads(_player_file.read_text(encoding='utf-8'))
        # Handle both direct player_object and wrapped payload structure
        _player_obj = _data.get('player_object') or _data.get('payload', {}).get('player_object')
        if _player_obj:
            logger.info('Loaded captured player data without currency overrides')
            return _player_obj
    except Exception as _e:
        logger.error('Failed to load captured player data: %s', _e)
    return None

CAPTURES_PLAYER_DATA = _load_captured_player_data()

# Inject captured data into msm_playerdata only when an explicit local-test switch is enabled.
_inject_captured_player_data()
if CAPTURES_PLAYER_DATA and os.environ.get('ALLOW_CAPTURED_PLAYER_DATA', '0').lower() in {'1', 'true', 'yes', 'on'}:
    msm_playerdata._captured_player_data = CAPTURES_PLAYER_DATA
    logger.info('Injected captured player data into msm_playerdata for local test mode')
else:
    msm_playerdata._captured_player_data = None
    logger.info('Captured player data disabled; using real saved player state')
...
async def sfs_websocket(websocket: WebSocket):
    ws_id = id(websocket)
    # Per-connection state. Holds the logged-in username so each player reads and writes their own
    # save; without it every connection was served as the default account.
    _session = {}
    
    # Extract client IP address
    client_ip_addr = websocket.client.host if websocket.client else ""
    if client_ip_addr:
        _session["client_ip"] = client_ip_addr
        logger.debug(f"WebSocket connection from IP: {client_ip_addr}")
    
    _ACTIVE_WEBSOCKETS.add(ws_id)
    msm_handlers.ACTIVE_CONNECTION_COUNT = len(_ACTIVE_WEBSOCKETS)
    ...
    try:
        while True:
            ...
            # Only force captured data in explicit local-test mode; otherwise let the normal player loader handle real state.
            if _frame.command == 'gs_player' and CAPTURES_PLAYER_DATA and os.environ.get('ALLOW_CAPTURED_PLAYER_DATA', '0').lower() in {'1', 'true', 'yes', 'on'}:
                _results = [('gs_player', {'player_object': json.loads(json.dumps(CAPTURES_PLAYER_DATA))})]
                logger.info('gs_player: using captured data from Captures directory in local-test mode')
            else:
                try:
                    _results = msm_handlers.handle_command(_frame.command, _frame.params, _session)
                except Exception as _err:
                    logger.info('%s failed: %s', _frame.command, _err)
                    continue
            
            for _resp_cmd, _resp_payload in _results:
                # Never inject fake reward values for successful actions; let the real game logic determine rewards.
        ...
        _ACTIVE_WEBSOCKETS.discard(ws_id)
        msm_handlers.ACTIVE_CONNECTION_COUNT = len(_ACTIVE_WEBSOCKETS)
        # Deliberately does NOT restore from the session snapshot any more: that overwrote the
        # live save with an old copy every time anyone disconnected, discarding the whole session.
        logger.info('websocket disconnected (%s); save left intact', _session.get('username') or 'not logged in')
    except Exception as _f:
        _ACTIVE_WEBSOCKETS.discard(ws_id)
        msm_handlers.ACTIVE_CONNECTION_COUNT = len(_ACTIVE_WEBSOCKETS)
        # Suppress expected disconnect-related errors at INFO level
        _error_msg = str(_f).lower()
        if 'disconnect' in _error_msg or 'receive' in _error_msg:
            # Same reasoning as above: an unclean disconnect must not roll the save back either.
            logger.debug('websocket stopped: %s', _f)
        else:
...
for route in ['/purchases/steam/my_singing_monsters/InitiatePurchase.php']:
    app.add_api_route(route, initiate_purchase, methods=['POST'])
...
async def main():
    ...
    try:
        ...
        watchdog.cancel()
        _UVICORN_SERVERS.clear()
        # Shutting the server down must not rewrite player saves from a snapshot.
