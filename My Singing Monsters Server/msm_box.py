...
def _direct_place_setting():
    """Runtime toggle from timemsm_settings.json, read per call so it flips without a restart."""
    try:
        import msm_store
        return msm_store._setting("direct_place")
    except Exception:
        return False


# Prefix, not an exact list: the Celestial Dip (992) is CLASS_DIPSTER_ASTRAL, so an exact
# membership test on "CLASS_DIPSTER" missed it and sent the only 3x3 Dipster through the egg /
# nursery flow instead of placing it directly like every other Dipster.
_DIRECT_PLACEMENT_CLASS_PREFIXES = ("CLASS_DIPSTER",)
def requires_direct_placement_on_purchase(definition, island_type, island_has_nursery=False):
    # ALL_DIRECT_PLACE=1 makes every purchase skip the nursery/egg flow and hand the monster
    # straight to the player, the way Dipsters and the Underling/Composer/Celestial/Amber
    # islands already behave.
    if os.environ.get("ALL_DIRECT_PLACE") == "1" or _direct_place_setting():
        return True
    # The custom island type 10 is the one the client expects to behave like a direct-place
    # nursery-free island. If the save is being forced to 10, skip the nursery popup path and
    # place the egg directly instead.
    if island_type == 22:
        return True
    # 10/11/12/22 ship without a nursery by design, and THAT is the real reason their purchases
    # have to place directly. If the player has since built one, use it: the client names the
    # nursery explicitly in structure_id, and a direct-placed egg carries structure 0, which
    # never renders - the egg lands in the save but is invisible on the island.
    if island_type in (11, 12, 22) and not island_has_nursery:
        return True
    # NOTE: box monsters (Wubbox/Wublins) must NOT be forced to direct-place on purchase. Real
    # server captures show their eggs go to a NURSERY like any other egg (gs_buy_egg 238 = Wubbox
    # Earth Island -> structure 2; 0 of 10 real captures ever used structure 0). They only place
    # directly at HATCH time, which requires_direct_placement() already handles.
    return bool(definition and str(definition.get("class") or "").startswith(_DIRECT_PLACEMENT_CLASS_PREFIXES))
...
def append_boxed_egg(monster, boxed_monster_id):
    ...
    # No limit on adding monsters to boxes - allow unlimited additions
...
def monster_awakened_update(monster):
    """Return the standard gs_update_monster payload for a newly awakened box monster.

    The box-progress payload above is valid for the box UI, but the client rejects it when it
    arrives as a generic monster update after hatch. The server must send a monster-shape update
    that flips the awakened flags instead.
    """
    return {
        "user_monster_id": SFSLong(monster.get("user_monster_id", 0)),
        "awakened": True,
